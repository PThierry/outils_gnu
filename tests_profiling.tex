...
profiling (étude du profil d'exécution d'une tache, en terme d'appel de fonctions,
de consommation mémoire, etc).\\
...
compatibles avec les exigences d'entrée du logiciel. Ces dernières sont souvent définies sous forme d'un
...
\paragraph{}
Au travers de l'exécution des logiciels de test il peut être  intéressant
...
Le schéma \ref{fig:gprof} montre un arbre d'exécution d'un logiciel complet. Il indique l'arbre
...
\end{center}
\label{fig:gprof}
\caption{Exemple de graphe d'appel de fonctions résultant de l'exécution de GProf}
\end{figure}

\subsection{Mesurer la complexité d'un source: pmccabe}

\paragraph{}
comme vu précédemment,    compte  tenu    que   l'effort     de test    peut être
particulièrement coûteux, il est intéressant de savoir à l'avance quel
...
nombre de chemins d'exécution augmente, et plus le comportement du bloc devient complexe. Une
...
Cachegrind implémente un simulateur d'algorithmes de caches de type LRU\footnote{{\it Least Recently
Used}, remplace la ligne de cache la moins souvent utilisée} avec un cache à deux niveaux.

...
\paragraph{}
Construire un binaire n'est pas une opération simple.  Il suffit de regarder le temps qu'il a fallu  pour  passer  de
...
appels aux macros (déclaré au travers de l'instruction de preprocessing \texttt{\#define}) par le contenu de la macro.\\
Il en va de même pour l'inclusion des header, en prenant en  compte  des  problématiques  de  redondance  d'inclusion.

\item Construire les fichiers objets.\\Un fichier objet est créé à partir de la compilation d'un fichier source. Dans  ce  fichier
sont  déclarés  les différents symboles utilisés et définis dans le fichier source associé.  On y retrouve ainsi la
liste  des  fonctions implémentées et celles qui sont utilisées et non encore définies (comme celles implémentées dans
un autre fichier source et accessible via un header). Il en va de même pour les variables globales, les structures de
données, les classes, etc.

\item La construction des bibliothèques.\\Il s'agit de regrouper certains fichiers objet en un élément homogène :  une
bibliothèque.  Il en ressort un fichier avec l'extension ``{\tt .a}'', contenant l'ensemble des symboles des  fichiers
objet utilisés pour construire la bibliothèque.  Cette étape n'est pas toujours présente mais  est  assez  fréquente.

\item Le linkage.\\Une fois les fichiers objets et les bibliothèques construits, il s'agit d'en faire un tout cohérent.
On construit alors une table définissant les différentes sections du binaire (partie code, partie
{\tt data}, {\tt rodata},  etc.), on résout les symboles inconnus venant des bibliothèques tierces comme  la  libc,
et  on  construit les méta-données\footnote{Ensemble des informations sur le contenu du binaire,
...
éléments du binaire,  et  pour  pouvoir amorcer son exécution.
\end{enumerate}
...
\paragraph{}
Comme précisé ci-dessus, l'étape de préprocessing prend en compte toutes les  commandes  de  préprocessing  appellées
...
\paragraph{}
Une fois le traitement du préprocesseur effectué, l'ensemble des lignes de preprocessing sont
remplacées  par  le  code qui est derrière, et ce de manière récursive.  Ainsi, on remplace l'appel à {\tt stdio.h}  par
son  contenu,  et  dans  le contenu du header lui-même, les lignes de preprocessing sont  également  remplacées jusqu'à
ce  qu'il  n'en subsiste aucune.

\paragraph{}
Vu la taille du fichier résultant du traitement du préprocessing, ce dernier est mis en annexe.  On  constate  qu'il
...
s'agit, à partir des fichiers de sortie du préprocesseur, de générer des fichiers binaires. Un fichier binaire est créé
pour chaque fichier source, avec comme seule différence dans le nom l'extention du fichier, qui devient ``{\tt
.o}''\footnote{{\tt o} pour objet}.

\paragraph{}
Les fichiers objets contiennent directement du code binaire. Si un passage par le code assembleur devait être effectué,
il  aurait  lieu  pendant  la  construction  du  fichier  objet,  comme  une  étape   intermédiaire.

\paragraph{}
Comme vu précédemment, un fichier objet contient des symboles non résolus. Il s'agit par exemple d'appel à des
...
implémentés (cas des symboles marqués {\tt t} ou {\tt T}, ou non-définits (cas des symboles marqués
{\tt U}).

\paragraph{}
Du fait qu'il ne s'agisse ici que de la concaténation de la table des symboles de chaque fichier objet, on retrouve,
pour  un  symbole  donné  plusieurs  occurences  dont, au  maximum, une  seule  marquée  {\tt   T}.

\paragraph{}
Ainsi, en cherchant le symbole {\tt strlen} dans la libc, on retrouve une sortie ressemblant à la liste ci-dessous:
...
\paragraph{}
Les adresses indiquées dans la table des symboles sont des marqueurs d'offset dans la section de code du fichier
objet.\\
Il s'agit d'adresse relative au début de la section code. Lors de la construction du binaire, puis à
...
le fichier binaire. La manière dont sont structurées ces données et positionnés les différents élements du binaire
(les   différentes   sections,   cf    plus    loin)    définit    le    format    du    binaire.
...
\paragraph{}
Parmi les différentes sections, on retrouve:
\begin{itemize}
\item La section {\texttt .text}, contenant le code exécutable
\item La section {\texttt .data}, contenant les données accessibles en écriture. Il peut s'agir de variables globales
par exemple.
\item La section {\texttt .ro-data}, contenant les données accessibles en lecture seule. Il s'agit
entre autres des chaîne de caractères {\it brutes}, comme la variable suivante :\\
\begin{lstlisting}
...
fait de l'exécution du logiciel. Il s'agit du tas (heap) et de la pile (stack). Ces   sections
sont spécifiques au processus\footnote{Un processus est une tache {\it en cours d'exécution}, à
l'inverse d'un exécutable, qui n'est qu'un fichier}.

\paragraph{}
Lors du linkage, on ne retrouve plus qu'une seule section de code (section .text). Elle regroupe l'ensemble des
sections de code des fichiers objets. Du fait du regroupement, les offset des différents symboles définis dans les
fichiers objets changent en fonction de leur position dans la nouvelle section .text. Il s'agit de la première
...
\begin{enumerate}
\item On intègre le code de la bibliothèque dans le binaire. Dans ce cas, les symboles de la bibliothèque sont
intégrés comme autant de nouveaux symboles dans la table des symboles du binaire. Le code des fonctions de la
bibliothèque est  intégré  quant  à  lui  dans  la   section   {\texttt   .text}   du   binaire.\\
Il faut prendre note toutefois que le linker, loin d'être bête, n'ajoute que le code de la bibliothèque nécessaire
...
mapper en mémoire la bibliothèque. L'intérêt principal est de partager les ressources entre les divers processus.
...
\paragraph{}
Au travers de la commande {\tt gcc} sont exécutés à la fois le préprocesseur ({\tt cpp}), le compilateur ({\tt gcc})
et le linker ({\tt ld}). En conséquence, on fournit à gcc à la fois des options pour le preprocessing, pour la
...
plus particulièrement les options de preprocessing ou de linkage, il vaut alors mieux s'appuyer sur le manuel
...
{\textbf{-s}} &
        \begin{minipage}{0.7\linewidth}
        \vspace{2mm}
        Gcc génère un fichier assembleur. Selon les options d'optimisation, le compilateur peut générer un code
...
{\textbf{-W}} &
        \begin{minipage}{0.7\linewidth}
        \vspace{2mm}
        Cette option demande au compilateur d'afficher les warnings les plus fréquents. Cette option ne fait
        qu'afficher sur la sortie d'erreur ({\tt stderr}) les élément de programmation hasardeux les  plus  graves.
        \vspace{2mm}
        \end{minipage}\\
        \hline
%%%%%%%%%%%%%%%%%%%
{\textbf{-Wall}} &
        \begin{minipage}{0.7\linewidth}
        \vspace{2mm}
        Idem à {\tt -W}, mais prend en compte plus de Warnings. Il regroupe un sous-ensemble des différents
        warning  géré  par  gcc  (confer  le  manuel  de  gcc ({\tt man gcc}),   partie   {\it
        Warning   Options}).
        \vspace{2mm}
        \end{minipage}\\
        \hline
...
{\textbf{-g}} &
        \begin{minipage}{0.7\linewidth}
        \vspace{2mm}
        Lors de la compilation, intégrer les éléments de debug. Cela permet, lors du debugging de l'application durant
        son exécution, de pouvoir s'appuyer sur les symboles et les métainformations (nom réel des fonctions, etc.).
        Dans le cas contraire, même le nom des fonctions (qui ne sont en fin de compte que des
        adresses) n'est pas présent dans le binaire.
        \vspace{2mm}
        \end{minipage}\\
        \hline
%%%%%%%%%%%%%%%%%%%
{\textbf{-ggdb}} &
        \begin{minipage}{0.7\linewidth}
        \vspace{2mm}
        Les symboles de debug sont optimisés pour gdb. Les informations sont plus grandes car mieux exploitées par le
        debuggeur de la GNU.
        \vspace{2mm}
        \end{minipage}\\
        \hline
...
base de la compréhention de l'usage de la mémoire par les processus, et par extention de son impact sur ses
performances et sur sa sécurité.

\paragraph{}
Enfin, il est important de noter que de nouvelles options de compilation apparaissent au fur et à mesure des nouvelles
...
\paragraph{}
Seules   deux   options   de   linkage   sont    décrites    dans    le    tableau    \ref{tab:ld_opts}
\begin{table}
\begin{tabular}{|l|l|}
\hline
Option & Signification \\
{\textbf{-l}} &
        ...
        est implicite pour la libc, mais doit être fournie pour toutes les autres bibliothèques. Seul le nom de la
        bibliothèque,   sans   le   prefixe   {\it   lib}   doit   être   fourni.     Voici un
        exemple avec la libgtk (bibliothèque graphique sur laquelle se base le bureau Gnome) :\\
        \begin{lstlisting}
...
{\textbf{-L}} &
        \begin{minipage}{0.7\linewidth}
        Il arrive que la bibliothèque utilisée ne soit pas présente dans les répertoires par défaut des bibliothèques

...
Le script \ref{lst:mak_ctrl} contient une structure de contrôle s'appuyant sur la présence ou non
de la variable d'environnement {\tt MORE\_CFLAGS}. Si cette variable est présente dans
l'environnement d'exécution et possède la valeur {\tt 1}, alors {\tt make} entre dans la structure
de contrôle {\tt ifeq}. A l'inverse, si cette variable n'est pas présente ou ne vaut pas {\tt 1},
la structure de contrôle est ignorée.
...
définition d'une variable, dont la syntaxe est ressemblante à celle du shell {\it bash}.\\

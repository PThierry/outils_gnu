...
nécessaire, dans certains cas, de vérifier les capacités des outils de compilation afin de
...
simplifié ou automatisé. C'est là qu'interviennent les outils de configuration des sources et du
...
Cette documentation est parfois rédigée en \LaTeX, ce qui implique la présence du compilateur LaTeX
...
\section{Les limites de la configuration : problématique de la cross-compilation}

\paragraph{}
Il arrive fréquement qu'un projet source puisse être produit pour différentes architectures
matérielles. Cependant, on ne produit pas toujours un logiciel directement sur l'architecture
matérielle cible, la plupart du temps car les outils de production ne sont pas présents sur cette
architecture. Afin de simplifier le travail du mainteneur de l'applicatif, il existe des {\it
chaînes de cross-compilation}. Il s'agit de chaînes de compilation dont le but est de produire un
binaire non pas pour l'architecture sur laquelle elles s'exécutent mais pour une autre architecture
pour laquelle elles ont été faites. Ainsi, sur un PC de bureau (de type x86) il est possible de
produire des binaires pour du SPARC, de l'ARM, du MIPS, ou divers autres architectures. Ce type de
production est appellée {\it cross compilation}. Construire le système de production d'un logiciel
pour pouvoir supporter la cross-compilation nécessite quelques restrictions.
...
\hline
La présence de divers headers du langage de programmation du projet \\
\hline
Pour une bibliothèque donnée, la présence de diverses fonctions du langage de programmation du projet \\
\hline
...
\paragraph{}
Parmi toutes les capacités du système de configuration, certaines ne sont pas nécessairement bonnes
à utiliser. C'est entre autre le cas des informations sur la taille en octets des types ({\tt int},
{\tt long}, {\tt short}...).\\
En effet, lorsque l'on fait de la cross-compilation, c'est en général sur une plate-forme donnée pour une
autre. Par exemple, on cross-compile sur une architecture Intel IA32 pour un processeur ARM.

\paragraph{}
Lorsque le système de production récupère les informations sur les tailles des type de données, il le fait sur
la plate-forme locale, non sur la plate-forme cible. Il n'a d'ailleurs aucune possibilité de
connaître,  au moment de la compilation, les propriétés de la plate-forme cible. Ainsi, on considère
qu'un système de configuration des sources ne doit aider que pour vérifier les éléments nécessaires à
...
\section{Généralités sur la structure des autotools}
...
déclare le nom du projet, sa version, la structure des sources, la position des différents Makefiles
...
nommés {\tt Makefile.am} ({\it am} pour automake). Ces derniers ont une syntaxe proche du
Makefile, mais fournissent un certain nombre de mots clefs rendant plus simple la rédaction des
Makefiles.

Dans le chapitre précédent, un fichier Makefile type est décrit. Le script \ref{lst:mak_am} décrit
...
définies explicitement, mais construites à partir des informations fournies dans le fichier {\tt
...
\item distcheck : construit une archive, et vérifie son bon fonctionnement en la désarchivant et
en exécutant le fichier configure et les cibles check du répertoire où elle est désarchivée.
\item clean : détruit les fichiers temporaires (fichiers objets, etc.)
\item distclean : détruit les programmes et éléments finaux construits
...
normalement pas nécessaire à l'utilisateur de l'appeller, car le fichier configure est en effet
...
dans ce chapitre, comme l'indique la figure \ref{fig:autoconf}. Néanmoins, les éléments de ce
...
autotools sont très puissants, mais complexes à comprendre du fait de leur richesse.\\
